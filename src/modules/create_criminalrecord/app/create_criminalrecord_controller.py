...
class CreateCriminalRecordController:
    ...
    def __call__(self, request: HttpRequest) -> HttpResponse:
        try:
            ...
            crimes_type = [crime.value for crime in CRIME]
            crimes_entity = []
            ...
            for crime in crimes:
                ...
                if criminal_inside_crime is None:
                    raise MissingParameters("criminal")
                if criminal_inside_crime.get("id_criminal") is None:
                    raise MissingParameters("id_criminal")
                ...
                if criminal_inside_crime.get("gender") is None:
                    raise MissingParameters("gender")
                if criminal_inside_crime.get("region_criminal") is None:
                    raise MissingParameters("region_criminal")

                if type(criminal_inside_crime.get("gender")) is not str:
                    raise EntityError("Invalid Gender")
                if criminal_inside_crime.get("gender") not in [
                ...
                criminal_entity = Criminal(
                    id=criminal_inside_crime.get("id_criminal"),
                    name=criminal_inside_crime.get("name"),
                    description=criminal_inside_crime.get("description"),
                    region=criminal_inside_crime.get("region_criminal"),
            ...
            criminalrecord_response = self.createCriminalRecordUsecase(
                criminalrecord=criminalrecord
            )
