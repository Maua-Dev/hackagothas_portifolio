...
class Test_Criminal():
    ...
    def test_criminal_invalid_name(self):
        with pytest.raises(Exception):
            criminal = Criminal(name=1,  description="LINUX", gender=GENDER.MALE, region="Brazil")
            
    def test_criminal_invalid_description(self):
        with pytest.raises(Exception):
            criminal = Criminal(name="Furlan", description=1, gender=GENDER.MALE, region="Brazil")
            
    def test_criminal_invalid_gender(self):
        with pytest.raises(Exception):
            criminal = Criminal(name="Furlan", description="LINUX", gender=1, region="Brazil")
            
    def test_criminal_invalid_region(self):
        with pytest.raises(Exception):
            criminal = Criminal(name="Furlan", description="LINUX", gender=GENDER.MALE, region=1)